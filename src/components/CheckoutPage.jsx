...
import cartData from './Checkout/data.json';
...
const CheckoutPage = () => {
    ...
    const { items, summary } = cartData.cart;
    ...
    return (
        <div className='flex flex-col lg:flex-row justify-center mt-32'>
            ...
            <div className='border-gray-300'>
                <div className='mt-14 '>
                    {cartItems.map((item, index) => (
                        <div key={index} className='flex flex-row ml-16 mb-4 items-center' >
                            <img className='w-20 h-20 border-1 border-gray-300 shadow-sm rounded-lg' src={imageMap[index]} alt={item.name} />
                            <div className='flex flex-row w-full ml-6 justify-between items-center'>
                                <h4 className=' mr-24'>{item.name}</h4>
                                <p>${item.price}</p>
                            </div>
                        </div>
                    ))}
            ...
            <div class="border-t-2 lg:border-l-2 lg:border-t-0 border-gray-300">

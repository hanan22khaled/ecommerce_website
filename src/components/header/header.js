    ...
    <div className="header-sec">
      ...
      <div className="sec-part flex justify-center items-center px-4 py-1">
        ...
        <div className="right-sec-part flex items-center space-x-4">
          ...
          <span>
            Need help? Call Us: <span style={{ color: "#35AFA0", fontWeight: "bolder" }}>+020100</span>
          </span>
          ...
          <Menu as="div" className="relative inline-block">
            ...
            <MenuItems className="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white shadow-lg outline outline-1 outline-black/5">
              <div className="py-1">
                <MenuItem>
                  <Link to="#" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700">
          ...
          <Menu as="div" className="relative inline-block">
            ...
            <MenuItems className="absolute right-0 z-10 mt-2 w-35 origin-top-right rounded-md bg-white shadow-lg outline outline-1 outline-black/5">
              <div className="py-1">
                <MenuItem>
                  <Link to="#" className="block px-4 py-2 text-sm text-gray-700">SAR</Link>
                </MenuItem>
                <MenuItem>
                  <Link to="#" className="block px-4 py-2 text-sm text-gray-700">URE</Link>
                </MenuItem>
              </div>
            </MenuItems>
          </Menu>
      ...
      <div className="third-part flex items-center justify-center gap-3 w-full h-[70px] relative px-4">
        <img src="/images/market-logo.png" alt="logo" className="w-18 h-12 object-contain" />
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div ref={menuRef} className="absolute top-14 right-3 w-52 bg-white shadow-lg flex flex-col p-3 gap-2 z-20 rounded-lg">
            {links.map((link) => (
              <Link
                key={link.name}
                to={link.href}
                ...
                className={`text-left px-3 py-2 text-sm rounded-lg ${
                  active === link.name ? "bg-green-100 text-green-700" : "text-black hover:text-green-700"
                }`}
              >
      ...
      {/* Fourth Part */}
      <div className="fourth-part flex flex-row items-center justify-center  w-full  py-2 shadow-md">
        {/* All Categories */}
        <Menu as="div" className="relative inline-block hidden min-[891px]:block">
          ...
          <MenuItems className="absolute z-10 mt-2 w-40 origin-top-left rounded-md bg-white shadow-lg outline outline-1 outline-black/5">
            <div className="py-1">
              <Link to="#" className="block px-4 py-2 text-sm text-gray-700">link1</Link>
              <Link to="#" className="block px-4 py-2 text-sm text-gray-700">link2</Link>
              <Link to="#" className="block px-4 py-2 text-sm text-gray-700">link3</Link>
            </div>
          </MenuItems>
        </Menu>

        {/* Desktop Links */}
        <div className="hidden dist min-[891px]:flex items-center gap-2">
          {links.map((link) =>
            link.dropdown ? (
              <Menu as="div" key={link.name} className="relative inline-block">
                ...
                >
                  {link.name}
                  <ChevronDownIcon aria-hidden="true" className="-mr-1 size-4 text-gray-500" />
                </MenuButton>
                <MenuItems className="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg outline outline-1 outline-black/5">
                  <div className="py-1">
                    <Link to="/" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setActive("HOME")}>Link 1</Link>
                    <Link to="/" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setActive("HOME")}>Link 2</Link>
                    <Link to="/" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setActive("HOME")}>Link 3</Link>
                  </div>
                </MenuItems>
              </Menu>
            ) : (
